...
function Options:LoadDiscountSettings(container)
    local discountGroup = {
        type = "InlineGroup",
        title = "Auction Data Age Discounts",
        layout = "Flow",
        children = {
            {
                type = "CheckBox",
                ...
                relativeWidth = 1,
                settingInfo = {TSM.db.profile, "enableSuggestions"},
                tooltip = "Enable profit-based suggestions and sorting",
                callback = function() 
                    -- Force UI refresh when toggled
                    if TSM.GUI and TSM.GUI.UpdateST then
                        TSM.GUI.UpdateST(true)
                    end
                end,
            },
            {
    ...
    -- Initialize discount percentages if not set
    if not TSM.db.profile.discountPercentages then
        TSM.db.profile.discountPercentages = TSM:GetDefaultDiscounts()
    end
    
    -- Add discount sliders for each profession
    local professions = {
        { key = "disenchant", name = "Disenchantable Items (Gear)" },
        { key = "milling", name = "Millable Items (Herbs)" },
        { key = "prospect", name = "Prospectable Items (Ore)" }
    }
    
    local ageCategories = {
        { key = "fresh", name = "Fresh Data (0-2 days)" },
        { key = "warning", name = "Aged Data (3-7 days)" },
        { key = "old", name = "Old Data (7+ days)" }
    }
    
    for _, profession in ipairs(professions) do
        -- Profession label with clearer description
        tinsert(discountGroup.children, {
            type = "Label",
            text = "|cFFFFFF00" .. profession.name .. "|r",
            relativeWidth = 1,
        })
        
        for _, ageCat in ipairs(ageCategories) do
            tinsert(discountGroup.children, {
				type = "Slider",
				label = ageCat.name,
				relativeWidth = 0.48,
				...
				ispercent = true,
				settingInfo = {
					TSM.db.profile.discountPercentages[profession.key], 
					ageCat.key,
					function(value) 
						-- Store as whole number (no conversion needed)
						return value
					end,
					function(value)
						-- Display as whole number (no conversion needed)
						return value or 0
					end
				},
				...
				callback = function()
					...
					if TSM.GUI and TSM.GUI.private then
						TSM.GUI.private.discountSettingsChanged = true
						-- Reset the flag after 3 seconds using old API
						TSMAPI:CreateTimeDelay("discountResetDelay", 3, function()
							if TSM.GUI and TSM.GUI.private then
								TSM.GUI.private.discountSettingsChanged = false
							end
						end)
					end
					
					-- Clear all caches
					if TSM.GUI and TSM.GUI.ClearDestroyCache then
						TSM.GUI.ClearDestroyCache()
					end
					if TSM.GUI and TSM.GUI.ClearTooltipCache then
						TSM.GUI.ClearTooltipCache()
					end
        ...
        -- Add some spacing between professions
        tinsert(discountGroup.children, {
            type = "Spacer",
            relativeWidth = 1,
            height = 8,
    ...
    tinsert(discountGroup.children, {
    type = "Button",
    text = "Reset Discounts to Default",
    relativeWidth = 0.5,
    callback = function()
        -- Completely clear and reset
        TSM.db.profile.discountPercentages = nil
        TSM.db.profile.discountPercentages = TSM:GetDefaultDiscounts()
        
        -- Set discount changed flag
        if TSM.GUI and TSM.GUI.private then
            TSM.GUI.private.discountSettingsChanged = true
            -- Reset the flag after 3 seconds using old API
            TSMAPI:CreateTimeDelay("discountResetDelay", 3, function()
        ...
        -- Simple approach: just print a message and let user manually refresh
        TSM:Print("Discounts reset to defaults. Close and reopen TSM Destroying options to see updated values.")
    end,
})
	...
	local page = {
		{
			...
			fullHeight = true,
			children = {
				Options:LoadDiscountSettings(container),
				{
					type = "InlineGroup",
					...
					children = {
						{
								...
								callback = function() 
									-- Clear the destroy cache to force re-evaluation of all items
									if TSM and TSM.IsDestroyable then
										-- This will clear the cache in the main module
										TSM.destroyCache = {}
									end
									
									-- Force an immediate update of the destroying frame
									if TSM.GUI and TSM.GUI.UpdateST then
										TSM.GUI.UpdateST(true)
									end
								end,
