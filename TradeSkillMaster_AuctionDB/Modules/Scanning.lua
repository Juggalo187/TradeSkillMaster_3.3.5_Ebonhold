...
local function FullScanCallback(event, ...)
	if event == "SCAN_PAGE_UPDATE" then
		...
		if (page >= 1) and (total > page) then
			...
			local seconds_remaining = max(1, floor((total - page) * seconds_per_page + 0.5))

			-- Convert the "elapsed / estimated" seconds into hours, minutes and seconds.
			local elapsed_h, elapsed_m, elapsed_s = TSMAPI:SecondsToHMS(seconds_elapsed)
			local total_h, total_m, total_s = TSMAPI:SecondsToHMS(seconds_total_estimate)
			local remaining_h, remaining_m, remaining_s = TSMAPI:SecondsToHMS(seconds_remaining)
			
			time_estimate_str = format(" (%s / ~%s)", TSMAPI:FormatHMS(elapsed_h, elapsed_m, elapsed_s), TSMAPI:FormatHMS(total_h, total_m, total_s))
			time_remaining_str = format(" - %s left", TSMAPI:FormatHMS(remaining_h, remaining_m, remaining_s))
		end

		-- Calculate progress bar from 0-100%.
		local progress_bar = min(100 * progress_float, 100)

		-- Display the progress bar with the time estimate and time remaining.
		TSM.GUI:UpdateStatus(format(L["Scanning page %s/%s"], page, total) .. time_estimate_str .. time_remaining_str, progress_bar)
